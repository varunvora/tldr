...
remove_rate=0.05
unk_rate=0.03
lower_lim=0.3 ##starting percentage where errors should be introduced
upper_lim=0.9 ##ending percentage after wich error should be avoided 
summary = extractive_summarizer(article, word_score_dictionary, sentence_count = 4, log_probability = True)
lsum=list(summary.split(" "));
print(len(lsum))
swapping_indexes=random.sample(range(int(0.3*len(lsum)),int(0.90*len(lsum))),int(swap_erate*len(lsum)))
print(swapping_indexes)
for i in swapping_indexes:
    temp=lsum[i]
    lsum[i]=lsum[i+1]
    lsum[i+1]=temp
swapping_indexes=random.sample(range(int(0.3*len(lsum)),int(0.90*len(lsum))),int(remove_rate*len(lsum)))
for i in swapping_indexes:
    lsum[i]=""
swapping_indexes=random.sample(range(int(0.3*len(lsum)),int(0.90*len(lsum))),int(remove_rate*len(lsum)))
for i in swapping_indexes:
